...
import { loadRolePageApi, removeRoleApi } from '#/api';
import { useDictionaryStore } from '#/store/dictionary';

import ConfigPermissionPage from './config-permission.vue';
...
const gridOptions: VxeGridProps<any> = {
  ...createGridOptions(loadRolePageApi),

  columns: [
    { title: '序号', type: 'seq', width: 50 },
    { align: 'left', type: 'checkbox', width: 50 },
    { field: 'roleCode', title: $t('permission.role.field.roleCode') },
    { field: 'roleName', title: $t('permission.role.field.roleName') },
    {
      field: 'status',
      title: $t('permission.role.field.status'),
      slots: { default: 'status' },
    },
    { field: 'remark', title: $t('permission.role.field.remark') },
    {
      field: 'action',
      fixed: 'right',
...
const [ConfigPermissionViewer, configPermissionViewerApi] = useVbenDrawer({
  connectedComponent: ConfigPermissionPage,
});
function editConfigPermission(rowData: any) {
  configPermissionViewerApi.setData({
    // 表单值
    values: rowData,
  });
  configPermissionViewerApi.open();
}
</script>
<template>
  <Page auto-content-height>
    <Grid>
      ...
      <template #action="{ row }">
        ...
        <Button class="mr-2 p-0" type="link" @click="editConfigPermission(row)">
          {{ $t('system.actions.configPermission') }}
        </Button>
