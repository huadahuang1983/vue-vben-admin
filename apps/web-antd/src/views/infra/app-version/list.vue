...
import { loadAppVersionPageApi, removeAppVersionApi } from '#/api';
    ...
    proxyConfig: {
      ajax: {
        query: async ({ page }, formValues) => {
          return await loadAppVersionPageApi({
...
function onCreate() {
  formDrawerApi.setData({ values: { isForceUpdate: false } }).open();
}

const onDelete = (removeRecords: any[]) => {
  const recordIds = removeRecords.map((item) => item.appVersionId);
  removeAppVersionApi(recordIds).then(() => {
    gridApi.query();
  });
};
