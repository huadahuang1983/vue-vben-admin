...
import { loadRegisteredClientPageApi, removeRegisteredClientApi } from '#/api';
    ...
    proxyConfig: {
      ajax: {
        query: async ({ page }, formValues) => {
          return await loadRegisteredClientPageApi({
...
const onDelete = (removeRecords: any[]) => {
  const recordIds = removeRecords.map((item) => item.id);
  removeRegisteredClientApi(recordIds).then(() => {
    gridApi.query();
  });
};
