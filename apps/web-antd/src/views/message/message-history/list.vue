    ...
    proxyConfig: {
      ajax: {
        query: async ({ page }, formValues) => {
          return await loadMessageHistoryPageApi({
...
const onDelete = (removeRecords: any[]) => {
  const recordIds = removeRecords.map((item) => item.messageHistoryId);
  removeMessageHistoryApi(recordIds).then(() => {
    gridApi.query();
  });
};
