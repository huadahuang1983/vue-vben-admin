...
import { bindPhoneNumberApi, sendVerifyCodeApi } from '#/api';

defineOptions({ name: 'BindPhoneNumber' });
  ...
  },
  {
    ...
    componentProps: {
      ...
      handleSendCode: async () => {
        ...
        sendVerifyCodeApi({ username: values.phoneNumber });
      },
    },
...
async function handleSubmit(values: Record<string, any>) {
  loading.value = true;
  bindPhoneNumberApi(values).finally(() => {
    loading.value = false;
  });
}
...
const [Modal, modalApi] = useVbenModal({
  ...
  title: $t('profile.securitySetting.bindPhoneNumber'),
});
</script>
